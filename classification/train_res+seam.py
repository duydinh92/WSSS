...
from network.resnet import ResNet_SEAM
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tag', default='train_res18+seam_input512', type=str)
    ...
    model = ResNet_SEAM(args.num_classes, pretrained=True)
    ...
    def evaluate(loader):
        ...
        with torch.no_grad():
            ...
            for step, (images, labels, gt_masks) in enumerate(loader):
                ...
                cams = model.seam_forward(images, mode='val')
                cams = cams[:,1:,:,:]
    ...
    for iteration in range(max_iteration):
        ...
        (score, cam, cam_rv), (affine_score, affine_cam, affine_cam_rv) = model.seam_forward(images, mode='train')
        matrix = model.get_affine_matrix()
        score = score.unsqueeze(2).unsqueeze(3)
        affine_score = affine_score.unsqueeze(2).unsqueeze(3)
        
        loss_rvmin1 = adaptive_min_pooling_loss((cam_rv*labels)[:,1:,:,:])
        cam = F.interpolate(max_norm(cam),scale_factor=scale_factor,mode='bilinear',align_corners=False)*labels
        cam_rv1 = F.interpolate(max_norm(cam_rv),scale_factor=scale_factor,mode='bilinear',align_corners=False)*labels

        loss_rvmin2 = adaptive_min_pooling_loss((affine_cam_rv*labels)[:,1:,:,:])
        affine_cam = max_norm(affine_cam)*labels
        affine_cam_rv = max_norm(affine_cam_rv)*labels
        ...
        tensor_ecr1 = torch.abs(max_onehot(affine_cam.detach()) - K.geometry.transform.warp_perspective(cam_rv, matrix, (hs, ws), align_corners=False)*labels)
        tensor_ecr2 = torch.abs(max_onehot(K.geometry.transform.warp_perspective(cam, matrix, (hs, ws), align_corners=False).detach())*labels - affine_cam_rv)
