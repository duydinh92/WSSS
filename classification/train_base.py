...
from network.densenet121 import DenseNet121
import network
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tag', default='train_baseline', type=str)
    ...
    model = DenseNet121(args.num_classes, pretrained=True)
    param_groups = model.get_parameter_groups(print_fn=None)
    load_model_fn()
    # model = network.resnet38_SEAM.Net()
    # model.load_state_dict(torch.load('/content/drive/MyDrive/WSSS/SEAM/resnet38_SEAM.pth'))
    ...
    for iteration in range(max_iteration):
        ...
        # cam, cam_rv = model(images)
        # score = F.adaptive_avg_pool2d(cam_rv, (1,1))
        # loss = class_loss_fn(score[:,1:,:,:], labels).mean()
